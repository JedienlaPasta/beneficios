...
export const DynamicFieldsRenderer = ({
  ...
  const getVisibleOptions = (field: DynamicFieldSchema) => {
    ...
    if (categoria === "Adulto") {
      return field.opciones.filter(
        (option) =>
          ADULT_SIZES.has(option.trim()) ||
          option.includes("Adulto") ||
          !CHILD_SIZES.has(option.trim()),
      );
    }
    ...
    if (categoria === "Infantil") {
      return field.opciones.filter(
        (option) =>
          CHILD_SIZES.has(option.trim()) || !ADULT_SIZES.has(option.trim()),
      );
    }

    // Si no hay categoría, se muestra todo
    return field.opciones;
  };
  ...
  return (
    <div className="grid grid-cols-1 gap-3">
      {schema.map((field) => {
        // Ociones visibles para este campo específico
                  ...
                  <AnimatePresence>
                    {fieldDropdownName === field.nombre && (
                      <motion.div
                        ...
                        className="absolute left-0 top-full z-50 mt-1 max-h-60 w-full overflow-auto rounded-md border border-slate-200 bg-white p-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                      >
