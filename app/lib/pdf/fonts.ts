...
export async function registerGeistFonts(pdfMake: PdfMakeBrowser) {
  if ((pdfMake as unknown as { __geistRegistered?: boolean }).__geistRegistered)
    return;
  ...
  const [regularRes, boldRes, blackRes, mediumRes] = await Promise.allSettled([
    fetch(regularUrl),
    fetch(boldUrl),
    fetch(blackUrl),
    fetch(mediumUrl),
  ...
  const [regularBuf, boldBuf, blackBuf, mediumBuf] = await Promise.all([
  ...
  (pdfMake as unknown as { __geistRegistered?: boolean }).__geistRegistered =
    true;
}
