...
export async function importXLSXFile(formData: FormData): Promise<FormState> {
  try {
    ...
    if (fileName && !isNaN(chunkIndex) && !isNaN(totalChunks)) {
      ...
      // For the final chunk, process the data but return immediately
      // to avoid timeout, and continue processing in the background
      if (chunkIndex === totalChunks - 1) {
        // Use waitUntil for background processing in Edge Runtime
        // or setTimeout for Node.js runtime to avoid blocking the response
        setTimeout(async () => {
          try {
        ...
        return {
          success: true,
          message:
            "Procesamiento iniciado en segundo plano. Los datos estarán disponibles en breve.",
        };
      }
    }

    // For non-chunked uploads or fallback
    return {
      success: true,
